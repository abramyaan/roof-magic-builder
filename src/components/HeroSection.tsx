  ...
  return (
    <section id="hero" className="relative min-h-screen flex items-center pt-32 pb-20">
      ...
      <div className="absolute inset-0 z-0">
        <img 
          src={heroImg} 
          alt="Кровельные работы в Москве и МО" 
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            <Button variant="hero" size="lg" className="w-full sm:w-auto" asChild>
              <a href="#quiz">
                ...
                <span className="hidden md:inline">Оставить заявку на бесплатный расчет</span>
              </a>
            </Button>
          </div>

          {/* Плашки на мобилке — в потоке под кнопкой */}
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            transition={{ delay: 1, duration: 0.5 }}
            className="flex flex-wrap gap-4 mt-10 md:hidden"
          >
            <div className="flex items-center gap-3 bg-black/30 backdrop-blur-md border border-white/10 p-2 px-4 rounded-xl shadow-lg">
              <div className="flex items-center justify-center w-8 h-8 bg-[#f33] rounded-full text-white font-bold text-lg flex-shrink-0">Я</div>
              <div>
                <div className="flex items-center gap-1">
                  <span className="text-white font-bold leading-none text-lg">4.9</span>
                  <Star className="w-3 h-3 fill-yellow-400 text-yellow-400" />
                </div>
                <p className="text-white/60 text-[10px] uppercase tracking-wider font-medium whitespace-nowrap">Яндекс Карты</p>
              </div>
            </div>
            <div className="flex items-center gap-3 bg-black/30 backdrop-blur-md border border-white/10 p-2 px-4 rounded-xl shadow-lg">
              ...
              <div>
                ...
                <p className="text-white/60 text-[10px] uppercase tracking-wider font-medium whitespace-nowrap">Авито отзывы</p>
              </div>
            </div>
          </motion.div>
        </motion.div>
      ...
      {/* Плашки на ПК — absolute к section, вне container, не перекрывают кнопку */}
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        transition={{ delay: 1, duration: 0.5 }}
        className="hidden md:flex flex-wrap gap-4 absolute bottom-4 left-4 z-20"
      >
        <div className="flex items-center gap-3 bg-black/30 backdrop-blur-md border border-white/10 p-2 px-4 rounded-xl shadow-lg">
          <div className="flex items-center justify-center w-8 h-8 bg-[#f33] rounded-full text-white font-bold text-lg flex-shrink-0">Я</div>
          <div>
